...
void ringMeter::update(int _temp, int usage)
{

  while (1)
  {
    if (_temp == this->temp && usage == this->usage)
      break;
    ...
    if (_temp != this->temp)
    {
      ...
      tft.drawArc(x, y, r, r - thickness, 30, _angle, color, TFT_BLACK);
      tft.drawArc(x, y, r, r - thickness, _angle, 330, TFT_BLACK, DARKER_GREY);
      this->angle = _angle;
    }
...
int CU, CT, CF, GU, GT, GF, RT, RU;

void setup(void)
{
  Serial.begin(115200);
  SerialBT.begin("PC Monitor");
  pinMode(22, OUTPUT);
  digitalWrite(22, 1);
  tft.begin();
  tft.setRotation(3);
...
void loop()
{
  if (SerialBT.available())
  {
    ...
    {
      if (doc.containsKey("CU"))
        CU = doc["CU"].as<int>();
      if (doc.containsKey("CT"))
        CT = doc["CT"].as<int>();
      if (doc.containsKey("CF"))
        CF = doc["CF"].as<int>();
      if (doc.containsKey("GU"))
        GU = doc["GU"].as<int>();
      if (doc.containsKey("GT"))
        GT = doc["GT"].as<int>();
      if (doc.containsKey("GF"))
        GF = doc["GF"].as<int>();
      if (doc.containsKey("RT"))
        RT = doc["RT"].as<int>();
      if (doc.containsKey("RU"))
        RU = doc["RU"].as<int>();

      Serial.println("Setting CPU");
      cpuMeter.update(CU, CT);
      Serial.println("Setting GPU");
      gpuMeter.update(GU, GT);
      Serial.println("Setting RAM");
      ramMeter.update(RU, RT);
      Serial.println("Setting FQ");
      // updateFreq(CF, GF);
    }
    else
    {
...
//   {
//     maxRadius = tft.width();
//     if (tft.height() < maxRadius)
//       maxRadius = tft.height();
